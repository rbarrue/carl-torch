...
def percentile_range(value):
    ...
    if value > 100. or value < 0.:
        raise argparse.ArgumentTypeError("%(value)s should be between 0-100, with float precision allowed." )
    return value
#################################################

# Argument parsing main function for training step
def arg_handler_train():
    parser = argparse.ArgumentParser(usage="%(prog)s [opts]")
    ...
    parser.add_argument('-nb','--nbins',type=int,dest='nbins',default=100,help='number of bins in evaluation plots') # RB
    ...
    parser.add_argument('--do_early_stopping', action='store_true', dest='do_early_stopping', default=False, help='use early stopping.')
    parser.add_argument('--early_stopping_patience', action='store', dest='early_stopping_patience', default=30, help='number of epochs after early stopping condition is met before the training is stopped.')
